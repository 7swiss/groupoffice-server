 ...
 */
abstract class RecordTest extends DataModel {
...
//			}
	...
	private function castDatabaseAttributes() {
		foreach ($this->getColumns() as $colName => $column) {			
			if(isset($this->$colName)) {
				$this->$colName = $this->loadedDbAttributes[$colName] = $column->dbToRecord($this->$colName);				
			}else
			{
				$this->loadedDbAttributes[$colName] = null;
			}
		}		
	...
	private function setLoadedDbAttributes() {
		foreach ($this->getColumns() as $colName => $column) {			
			if(isset($this->$colName)) {
				$this->$colName = $this->loadedDbAttributes[$colName] = $this->$colName;				
			}else
			{
				$this->loadedDbAttributes[$colName] = null;
			}
	...
	/**
	 * When a relation model is joined on a find action, We can select the model 
	 * attributes so we don't have to query it seperately later.
	 * eg. $contact->addressbook will work without an additional select query.
	 * 
	 * For example when you join the company on a contact:
	 * 
	 * <code>
	 * Contact::find((new Query)->joinRelation('company',['name']));
	 * </code>
	 * 
	 * this will return:
	 * <code>
	 ...
	 * </code>
	 * 
	 * {@see Query::joinRelation()}
	 */
	private function extractJoinedRelations() {
...
//		IFW::app()->debug($this->getClassName()."::__get($name)");
//		IFW::app()->debug($this->attributes);
...
//		}
	...
	private function setJoinedRelationAttribute($name, $value) {
		...
		$currentRecord->loadingFromDatabase = true;
	...
	public function isModified($attributeOrRelationName = null) {
		if (!isset($attributeOrRelationName)) {
			...
			foreach($this->loadedDbAttributes as $colName => $loadedValue) {
		...
		foreach ($attributeOrRelationName as $a) {			
			if(isset($this->loadedDbAttributes[$a]) && $this->loadedDbAttributes[$a] !== $a) {
				return true;
			}
	...
	public function reset($attributeName = null) {
		...
		if(!isset($attributeName)) {
			$attributeName = array_keys($this->loadedDbAttributes);
		...
		foreach($attributeName as $a) {
			$this->$a = $this->loadedDbAttributes[$a];
		}
	}
...
//		}
		return $this->loadedDbAttributes[$attributeName];
	}
...
//		var_dump($this->loadedDbAttributes);
		
		foreach($this->loadedDbAttributes as $colName => $loadedValue) {
			if($this->$colName !== $loadedValue) {
				$modified[] = $loadedValue;
			}
		}
		return $modified;
	}
...
//			IFW::app()->debug("Last insert ID for ".$this->getClassName().": ".$lastInsertId);			
	...
	protected function update() {		
		...
		if ($this->getColumn('modifiedAt') && !$this->isModified('modifiedAt')) {
			$this->modifiedAt = gmdate(\IFW\Util\DateTime::FORMAT_API);
		}
		...
		foreach (array_keys($this->getModifiedAttributes()) as $colName) {
			$i++;
			$tag = ':attr'.$i;
			$updates[] = "`$colName` = " . $tag;
...
//					throw new \Exception($attr['colName'].' attribute mustbe scalar. Given value: '.var_export($value, true));
//				}
...
//	}
	...
	public function truncateModifiedAttributes() {
		foreach($this->getModifiedAttributes() as $attributeName => $oldValue) {
			$this->attributes[$attributeName] = mb_substr($this->attributes[$attributeName], 0, $this->getColumn($attributeName)->length);
		}
	}
