 ...
 */
class Mailbox extends \IFW\Data\Model {
		...
		for ($i = 0, $c = count($arr); $i < $c; $i++) {
			...
			if (substr($value, 0, 1) == '(') {
				...
				do {
					...
					if(!isset($arr[$i])) {
						IFW::app()->debug("Abnormal structure encountered: ".$response);
						break;
					}
		...
		if(!empty($headers)){
			...
			if (isset($attr['date'])) {
				...
				try {
					...
					IFW::app()->debug("Failed to parse date: ".$e->getMessage());
